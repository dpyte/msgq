...
std::string get_ipc_path(const std::string &name) {
	std::string path = "/tmp/";
	if (char *prefix = std::getenv("WATCHDOG_PREFIX")) {
		path += std::string(prefix) + "_";
	}
	return path + "visionipc_" + name;
}

VisionIpcServer::VisionIpcServer(const std::string &name, cl_device_id device_id, cl_context ctx)
	: device_id(device_id), ctx(ctx), name(name) {
